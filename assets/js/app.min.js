...
(function(){"use strict";const s=(e,t=document)=>t.querySelector(e),f=(e,t=document)=>Array.from(t.querySelectorAll(e)),w=window.matchMedia("(prefers-reduced-motion: reduce)").matches,M=-80;function A(){const e=s("#loader");if(!e)return;let t=!1;const o=()=>{t||(t=!0,e.classList.add("is-done"),setTimeout(()=>e.remove(),500))},n=document.fonts?document.fonts.ready:Promise.resolve();Promise.race([n,new Promise(i=>setTimeout(i,1200))]).then(()=>requestAnimationFrame(o)),window.addEventListener("load",o),setTimeout(o,2e3)}let r=null;function T(){if(w||typeof Lenis=="undefined")return;r=new Lenis({duration:1.1,easing:t=>Math.min(1,1.001-Math.pow(2,-10*t)),smoothWheel:!0,touchMultiplier:1.6});function e(t){r.raf(t),requestAnimationFrame(e)}requestAnimationFrame(e),window.ScrollTrigger&&r.on("scroll",ScrollTrigger.update),window.mcLenis=r}function O(e){if(r)r.resize(),r.scrollTo(e,{offset:M,duration:1.3});else{const t=e.getBoundingClientRect().top+window.scrollY+M;window.scrollTo({top:t,behavior:w?"auto":"smooth"})}}function N(){f('a[href^="#"]').forEach(e=>{const t=e.getAttribute("href");!t||t==="#"||e.addEventListener("click",o=>{const n=s(t);n&&(o.preventDefault(),L(),setTimeout(()=>O(n),e.closest(".mobile-menu")?260:0),history.replaceState(null,"",t))})})}function q(){const e=f("[data-spy]");if(!e.length)return;const t=e.map(i=>{const a=i.getAttribute("href"),l=s(a);return l?{link:i,section:l,id:a}:null}).filter(Boolean);if(!t.length)return;let o=null;const n=()=>{const i=window.scrollY-M+8;let a=null;for(const c of t)c.section.offsetTop<=i&&(a=c);window.innerHeight+window.scrollY>=document.body.scrollHeight-4&&(a=t[t.length-1]);const l=a?a.id:null;l!==o&&(o=l,e.forEach(c=>c.classList.toggle("is-active",!!l&&c.getAttribute("href")===l)))};window.addEventListener("scroll",n,{passive:!0}),window.addEventListener("resize",n),n()}function D(){const e=s("#nav");if(!e)return;let t=0;const o=()=>{const n=window.scrollY;e.classList.toggle("nav--scrolled",n>40);const i=e.dataset.menuOpen==="true";e.classList.toggle("nav--hidden",!i&&n>700&&n>t),t=n};window.addEventListener("scroll",o,{passive:!0}),o()}const S=()=>s("#mobileMenu"),k=()=>s("#burger");function P(){const e=S(),t=k(),o=s("#nav");if(!e)return;e.classList.add("is-open"),o.dataset.menuOpen="true",o.classList.remove("nav--hidden"),t.setAttribute("aria-expanded","true"),t.setAttribute("aria-label","Menu sluiten"),r?r.stop():document.body.classList.add("menu-locked");const n=s(".mobile-menu-link",e);n&&setTimeout(()=>n.focus({preventScroll:!0}),320)}function L(){const e=S(),t=k(),o=s("#nav");!e||!e.classList.contains("is-open")||(e.classList.remove("is-open"),o.dataset.menuOpen="false",t.setAttribute("aria-expanded","false"),t.setAttribute("aria-label","Menu openen"),document.body.classList.remove("menu-locked"),r&&(r.start(),r.resize()))}function I(){const e=k(),t=S();!e||!t||(e.addEventListener("click",()=>t.classList.contains("is-open")?L():P()),document.addEventListener("keydown",o=>{o.key!=="Escape"||!t.classList.contains("is-open")||(L(),e.focus())}),t.addEventListener("keydown",o=>{if(o.key!=="Tab"||!t.classList.contains("is-open"))return;const n=f("a[href], button",t).filter(l=>l.offsetParent!==null);if(!n.length)return;const i=n[0],a=n[n.length-1];o.shiftKey&&document.activeElement===i?(o.preventDefault(),a.focus()):!o.shiftKey&&document.activeElement===a&&(o.preventDefault(),i.focus())}),window.addEventListener("resize",()=>{window.innerWidth>=1024&&L()}))}function R(){const e=s("#heroVideo"),t=s("#heroPoster");if(!e||!t)return;const o=e.dataset.vimeo,n=s("#heroSound");let i=null;function a({autoplay:p,muted:d}){const u=d?"ambient":"sound";if(i===u)return;const v=e.querySelector("iframe");v&&v.remove(),i=u;const g=new URLSearchParams({autoplay:p?"1":"0",muted:d?"1":"0",loop:"1",controls:d?"0":"1",playsinline:"1",title:"0",byline:"0",portrait:"0",transparent:"0",dnt:"1"}),m=document.createElement("iframe");m.src=`https://player.vimeo.com/video/${o}?${g}`,m.allow="autoplay; fullscreen; picture-in-picture",m.allowFullscreen=!0,m.title="MAIN CHARACTER Storytelling",m.loading="lazy",e.insertBefore(m,t);const W=()=>{e.classList.add("is-playing"),n&&(n.hidden=i!=="ambient")};function x(b){if(!/^https:\/\/player\.vimeo\.com/.test(b.origin))return;let E;try{E=typeof b.data=="string"?JSON.parse(b.data):b.data}catch{return}!E||!E.event||(E.event==="ready"?m.contentWindow.postMessage(JSON.stringify({method:"addEventListener",value:"play"}),"*"):E.event==="play"&&(window.removeEventListener("message",x),W()))}window.addEventListener("message",x)}const l=()=>a({autoplay:!0,muted:!1});t.addEventListener("click",l),n&&n.addEventListener("click",()=>{l(),n.hidden=!0});const c=navigator.connection,h=c&&(c.saveData||/(^|-)2g$/.test(c.effectiveType||""));if(w||h||!("IntersectionObserver"in window))return;const y=new IntersectionObserver(p=>{p[0].isIntersecting&&(y.disconnect(),a({autoplay:!0,muted:!0}))},{threshold:.1});y.observe(e)}function F(){const e=s("#countdown");if(!e)return;const t=new Date(e.dataset.deadline).getTime();if(Number.isNaN(t))return;const o=[e,...f("[data-deadline-mirror]")],n=c=>String(Math.max(0,c)).padStart(2,"0");function i(c,h,y){const p=String(h).split("");if(c.children.length!==p.length){c.textContent="",p.forEach(d=>{const u=document.createElement("span");u.className="cd-digit",u.innerHTML=`<b class="in">${d}</b>`,c.appendChild(u)});return}p.forEach((d,u)=>{const v=c.children[u],g=v.querySelector("b:not(.out)");if(!g||g.textContent===d)return;if(!y){g.textContent=d;return}g.classList.add("out");const m=document.createElement("b");m.className="in",m.textContent=d,v.appendChild(m),setTimeout(()=>g.remove(),520)})}let a=!0;function l(){var p;const c=t-Date.now();if(c<=0){o.forEach(u=>f("[data-cd]",u).forEach(v=>i(v,"00",!1)));const d=(p=e.parentElement)==null?void 0:p.querySelector(".eyebrow");d&&(d.textContent="De Deuren Zijn Open");return}const h=Math.floor(c/1e3),y={days:n(Math.floor(h/86400)),hours:n(Math.floor(h%86400/3600)),minutes:n(Math.floor(h%3600/60)),seconds:n(h%60)};o.forEach(d=>{f("[data-cd]",d).forEach(u=>i(u,y[u.dataset.cd],!a))}),a=!1,setTimeout(l,1e3)}l()}function $(){const e=f(".acc-item");if(!e.length)return;function t(n){const i=s(".acc-panel",n);s(".acc-trigger",n).setAttribute("aria-expanded","false"),n.dataset.open="false",window.gsap&&!w?gsap.to(i,{height:0,duration:.4,ease:"power2.inOut"}):i.style.height="0px"}function o(n){const i=s(".acc-panel",n);s(".acc-trigger",n).setAttribute("aria-expanded","true"),n.dataset.open="true",window.gsap&&!w?gsap.to(i,{height:"auto",duration:.5,ease:"power3.out",onComplete:()=>window.ScrollTrigger&&ScrollTrigger.refresh()}):i.style.height="auto"}e.forEach(n=>{s(".acc-trigger",n).addEventListener("click",()=>{const i=n.dataset.open==="true";e.forEach(a=>a!==n&&t(a)),i?t(n):o(n)})})}function B(){f("[data-carousel-next]").forEach(t=>t.addEventListener("click",()=>e(t.dataset.carouselNext,1))),f("[data-carousel-prev]").forEach(t=>t.addEventListener("click",()=>e(t.dataset.carouselPrev,-1)));function e(t,o){const n=s("#carousel-"+t);if(!n||!n.firstElementChild)return;const i=parseFloat(getComputedStyle(n).gap)||16;n.scrollBy({left:o*(n.firstElementChild.getBoundingClientRect().width+i),behavior:w?"auto":"smooth"})}}function z(){var n;const e=s("#videoModal"),t=s("#modalVideo");if(!e||!t)return;f("[data-video]").forEach(i=>{i.addEventListener("click",()=>{t.src=i.dataset.video,e.showModal(),r&&r.stop(),t.play().catch(()=>{})})});function o(){t.pause(),t.removeAttribute("src"),t.load(),e.open&&e.close(),r&&r.start()}(n=s("[data-modal-close]",e))==null||n.addEventListener("click",o),e.addEventListener("close",()=>{t.pause(),r&&r.start()}),e.addEventListener("click",i=>{i.target===e&&o()})}function V(){const e=s("#stickyCta"),t=s("#hero");!e||!t||!("IntersectionObserver"in window)||new IntersectionObserver(([o])=>e.classList.toggle("is-visible",!o.isIntersecting),{threshold:0,rootMargin:"-80% 0px 0px 0px"}).observe(t)}function Y(){window.matchMedia("(hover: none)").matches||f(".card-glow").forEach(e=>{e.addEventListener("mousemove",t=>{const o=e.getBoundingClientRect();e.style.setProperty("--mx",t.clientX-o.left+"px"),e.style.setProperty("--my",t.clientY-o.top+"px")})})}function H(){f("[data-cta]").forEach(e=>{e.addEventListener("click",()=>{const t={location:e.dataset.cta,value:125,currency:"EUR"};window.fbq&&fbq("track","InitiateCheckout",t),window.dataLayer&&dataLayer.push({event:"cta_click",...t})})})}function C(){A(),T(),N(),q(),D(),I(),R(),F(),$(),B(),z(),V(),Y(),H(),window.mcReady=!0,document.dispatchEvent(new CustomEvent("mc:ready",{detail:{lenis:r}}))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",C):C()})();
;
(function(){"use strict";const m=(e,t=document)=>t.querySelector(e),s=(e,t=document)=>Array.from(t.querySelectorAll(e)),h=window.matchMedia("(prefers-reduced-motion: reduce)").matches;function g(){if(typeof gsap=="undefined"||typeof ScrollTrigger=="undefined"||h)return;gsap.registerPlugin(ScrollTrigger),document.documentElement.classList.add("is-ready"),(document.fonts?document.fonts.ready:Promise.resolve()).then(x)}function x(){y(),T(),C(),S(),w(),L(),N(),A(),E(),ScrollTrigger.refresh()}function E(){s("[data-tloop]").forEach(e=>{const t=Array.from(e.children);if(!t.length)return;t.forEach(c=>{const l=c.cloneNode(!0);l.setAttribute("aria-hidden","true"),l.querySelectorAll("a, button").forEach(i=>i.setAttribute("tabindex","-1")),e.appendChild(l)});const o=e.scrollWidth/2,n=gsap.to(e,{xPercent:-50,duration:o/55,ease:"none",repeat:-1}),r=e.closest(".tloop")||e.parentElement;r.addEventListener("mouseenter",()=>gsap.to(n,{timeScale:0,duration:.4})),r.addEventListener("mouseleave",()=>gsap.to(n,{timeScale:1,duration:.4})),r.addEventListener("focusin",()=>n.pause()),r.addEventListener("focusout",()=>n.play()),ScrollTrigger.create({trigger:r,start:"top bottom",end:"bottom top",onToggle:c=>c.isActive?n.play():n.pause()})})}function y(){s('[data-split="lines"]').forEach(e=>{const t=b(e);if(!t.length)return;v(e,t);const o=s(".line-inner",e);gsap.set(o,{yPercent:118});const a=gsap.to(o,{yPercent:0,duration:1.15,ease:"power4.out",stagger:.085,paused:!0,onComplete:()=>s(".line-mask",e).forEach(r=>r.style.overflow="visible")}),n=new IntersectionObserver(r=>{r[0].isIntersecting&&(n.disconnect(),a.play())},{rootMargin:"0px 0px -12% 0px",threshold:0});n.observe(e)})}function b(e){const t=[],o=[],a=document.createTreeWalker(e,NodeFilter.SHOW_TEXT);for(;a.nextNode();)o.push(a.currentNode);let n=!1;o.forEach(i=>{if(!i.textContent.trim()){i.textContent.length&&(n=!0);return}const u=!!i.parentElement.closest("em, .accent"),f=document.createDocumentFragment();i.textContent.split(/(\s+)/).forEach(p=>{if(!p)return;if(!p.trim()){n=!0,f.appendChild(document.createTextNode(p));return}const d=document.createElement("span");d.style.display="inline-block",d.textContent=p,d.__accent=u,d.__spaceBefore=n,n=!1,f.appendChild(d),t.push(d)}),i.parentNode.replaceChild(f,i)});const r=[];let c=null,l=null;return t.forEach(i=>{const u=i.offsetTop;(l===null||Math.abs(u-l)>4)&&(c=[],r.push(c),l=u),c.push({text:i.textContent,accent:i.__accent,spaceBefore:i.__spaceBefore})}),r}function v(e,t){e.textContent="",t.forEach(o=>{const a=document.createElement("span");a.className="line-mask";const n=document.createElement("span");n.className="line-inner",o.forEach((r,c)=>{if(c>0&&r.spaceBefore&&n.appendChild(document.createTextNode(" ")),r.accent){const l=document.createElement("em");l.textContent=r.text,n.appendChild(l)}else n.appendChild(document.createTextNode(r.text))}),a.appendChild(n),e.appendChild(a)})}function T(){const e=s("[data-reveal]").filter(a=>!a.closest("[data-stagger]")),t=s("[data-stagger]").filter(a=>s("[data-reveal]",a).length);if(!e.length&&!t.length)return;const o=new IntersectionObserver(a=>{for(const n of a){if(!n.isIntersecting)continue;const r=n.target;o.unobserve(r);const c=r.hasAttribute("data-stagger"),l=c?s("[data-reveal]",r):r;gsap.to(l,{opacity:1,x:0,y:0,scale:1,duration:c?.9:1,ease:"power3.out",stagger:c?.08:0,delay:c?0:parseFloat(r.dataset.revealDelay)||0,overwrite:"auto"})}},{rootMargin:"0px 0px -10% 0px",threshold:0});e.forEach(a=>o.observe(a)),t.forEach(a=>o.observe(a))}function C(){s("[data-parallax]").forEach(e=>{const t=parseFloat(e.dataset.parallax)||.1;gsap.fromTo(e,{yPercent:-t*50},{yPercent:t*50,ease:"none",scrollTrigger:{trigger:e.parentElement||e,start:"top bottom",end:"bottom top",scrub:1}})})}function S(){s("[data-float]").forEach((e,t)=>{const o=e.dataset.float==="2"?-1:1;gsap.to(e,{y:o*60,x:o*34,duration:9+t*2.5,ease:"sine.inOut",repeat:-1,yoyo:!0}),gsap.to(e,{yPercent:26*o,ease:"none",scrollTrigger:{trigger:e.parentElement,start:"top bottom",end:"bottom top",scrub:1.4}})})}function w(){s("[data-count]").forEach(e=>{const t=parseFloat(e.dataset.count),o=e.dataset.countSuffix||"",a={value:0},n=new IntersectionObserver(r=>{r[0].isIntersecting&&(n.disconnect(),gsap.to(a,{value:t,duration:2,ease:"power2.out",onUpdate:()=>{e.textContent=Math.round(a.value).toLocaleString("nl-NL")+o}}))},{rootMargin:"0px 0px -8% 0px",threshold:0});n.observe(e)})}function L(){s(".timeline").forEach(e=>{const t=m("[data-timeline-progress]",e);t&&gsap.fromTo(t,{height:"0%"},{height:"100%",ease:"none",scrollTrigger:{trigger:e,start:"top 65%",end:"bottom 75%",scrub:.6}}),s("[data-timeline-item]",e).forEach(o=>{ScrollTrigger.create({trigger:o,start:"top 70%",end:"bottom 60%",onEnter:()=>o.classList.add("is-active"),onEnterBack:()=>o.classList.add("is-active")})})})}function N(){s("[data-marquee-scroll]").forEach(e=>{const t=parseFloat(e.dataset.marqueeScroll)||-10;gsap.fromTo(e,{xPercent:0},{xPercent:t,ease:"none",scrollTrigger:{trigger:e,start:"top bottom",end:"bottom top",scrub:1}})})}function A(){s("[data-meter]").forEach(e=>{const t=new IntersectionObserver(o=>{o[0].isIntersecting&&(t.disconnect(),gsap.to(e,{width:(parseFloat(e.dataset.meter)||0)+"%",duration:1.6,ease:"power3.out"}))},{threshold:0});t.observe(e)})}window.mcReady?g():document.addEventListener("mc:ready",g,{once:!0})})();
