...
(function(){"use strict";const s=(e,t=document)=>t.querySelector(e),u=(e,t=document)=>Array.from(t.querySelectorAll(e)),y=window.matchMedia("(prefers-reduced-motion: reduce)").matches,b=-80;function C(){const e=s("#loader");if(!e)return;let t=!1;const o=()=>{t||(t=!0,e.classList.add("is-done"),setTimeout(()=>e.remove(),500))},n=document.fonts?document.fonts.ready:Promise.resolve();Promise.race([n,new Promise(i=>setTimeout(i,1200))]).then(()=>requestAnimationFrame(o)),window.addEventListener("load",o),setTimeout(o,2e3)}let a=null;function x(){if(y||typeof Lenis=="undefined")return;a=new Lenis({duration:1.1,easing:t=>Math.min(1,1.001-Math.pow(2,-10*t)),smoothWheel:!0,touchMultiplier:1.6});function e(t){a.raf(t),requestAnimationFrame(e)}requestAnimationFrame(e),window.ScrollTrigger&&a.on("scroll",ScrollTrigger.update),window.mcLenis=a}function A(e){if(a)a.resize(),a.scrollTo(e,{offset:b,duration:1.3});else{const t=e.getBoundingClientRect().top+window.scrollY+b;window.scrollTo({top:t,behavior:y?"auto":"smooth"})}}function T(){u('a[href^="#"]').forEach(e=>{const t=e.getAttribute("href");!t||t==="#"||e.addEventListener("click",o=>{const n=s(t);n&&(o.preventDefault(),L(),setTimeout(()=>A(n),e.closest(".mobile-menu")?260:0),history.replaceState(null,"",t))})})}function O(){const e=u("[data-spy]");if(!e.length)return;const t=e.map(i=>{const c=i.getAttribute("href"),d=s(c);return d?{link:i,section:d,id:c}:null}).filter(Boolean);if(!t.length)return;let o=null;const n=()=>{const i=window.scrollY-b+8;let c=null;for(const r of t)r.section.offsetTop<=i&&(c=r);window.innerHeight+window.scrollY>=document.body.scrollHeight-4&&(c=t[t.length-1]);const d=c?c.id:null;d!==o&&(o=d,e.forEach(r=>r.classList.toggle("is-active",!!d&&r.getAttribute("href")===d)))};window.addEventListener("scroll",n,{passive:!0}),window.addEventListener("resize",n),n()}function N(){const e=s("#nav");if(!e)return;let t=0;const o=()=>{const n=window.scrollY;e.classList.toggle("nav--scrolled",n>40);const i=e.dataset.menuOpen==="true";e.classList.toggle("nav--hidden",!i&&n>700&&n>t),t=n};window.addEventListener("scroll",o,{passive:!0}),o()}const M=()=>s("#mobileMenu"),S=()=>s("#burger");function q(){const e=M(),t=S(),o=s("#nav");if(!e)return;e.classList.add("is-open"),o.dataset.menuOpen="true",o.classList.remove("nav--hidden"),t.setAttribute("aria-expanded","true"),t.setAttribute("aria-label","Menu sluiten"),a?a.stop():document.body.classList.add("menu-locked");const n=s(".mobile-menu-link",e);n&&setTimeout(()=>n.focus({preventScroll:!0}),320)}function L(){const e=M(),t=S(),o=s("#nav");!e||!e.classList.contains("is-open")||(e.classList.remove("is-open"),o.dataset.menuOpen="false",t.setAttribute("aria-expanded","false"),t.setAttribute("aria-label","Menu openen"),document.body.classList.remove("menu-locked"),a&&(a.start(),a.resize()))}function P(){const e=S(),t=M();!e||!t||(e.addEventListener("click",()=>t.classList.contains("is-open")?L():q()),document.addEventListener("keydown",o=>{o.key!=="Escape"||!t.classList.contains("is-open")||(L(),e.focus())}),t.addEventListener("keydown",o=>{if(o.key!=="Tab"||!t.classList.contains("is-open"))return;const n=u("a[href], button",t).filter(d=>d.offsetParent!==null);if(!n.length)return;const i=n[0],c=n[n.length-1];o.shiftKey&&document.activeElement===i?(o.preventDefault(),c.focus()):!o.shiftKey&&document.activeElement===c&&(o.preventDefault(),i.focus())}),window.addEventListener("resize",()=>{window.innerWidth>=1024&&L()}))}function D(){const e=s("#heroVideo"),t=s("#heroPoster");if(!e||!t)return;const o=e.dataset.vimeo,n=s("#heroSound");let i=null;function c({autoplay:r,muted:m}){const g=m?"ambient":"sound";if(i===g)return;const p=e.querySelector("iframe");p&&p.remove(),i=g;const f=new URLSearchParams({autoplay:r?"1":"0",muted:m?"1":"0",loop:"1",controls:m?"0":"1",playsinline:"1",title:"0",byline:"0",portrait:"0",transparent:"0",dnt:"1"}),l=document.createElement("iframe");l.src=`https://player.vimeo.com/video/${o}?${f}`,l.allow="autoplay; fullscreen; picture-in-picture",l.allowFullscreen=!0,l.title="MAIN CHARACTER Storytelling",l.loading="lazy",e.insertBefore(l,t);const w=()=>{e.classList.add("is-playing"),n&&(n.hidden=i!=="ambient")};function h(v){if(!/^https:\/\/player\.vimeo\.com/.test(v.origin))return;let E;try{E=typeof v.data=="string"?JSON.parse(v.data):v.data}catch{return}!E||!E.event||(E.event==="ready"?l.contentWindow.postMessage(JSON.stringify({method:"addEventListener",value:"play"}),"*"):E.event==="play"&&(window.removeEventListener("message",h),w()))}window.addEventListener("message",h)}const d=()=>c({autoplay:!0,muted:!1});t.addEventListener("click",d),n&&n.addEventListener("click",()=>{d(),n.hidden=!0}),t.addEventListener("pointerenter",()=>{const r=document.createElement("link");r.rel="preconnect",r.href="https://player.vimeo.com",document.head.appendChild(r)},{once:!0})}function R(){const e=s("#countdown");if(!e)return;const t=new Date(e.dataset.deadline).getTime();if(Number.isNaN(t))return;const o=[e,...u("[data-deadline-mirror]")],n=r=>String(Math.max(0,r)).padStart(2,"0");function i(r,m,g){const p=String(m).split("");if(r.children.length!==p.length){r.textContent="",p.forEach(f=>{const l=document.createElement("span");l.className="cd-digit",l.innerHTML=`<b class="in">${f}</b>`,r.appendChild(l)});return}p.forEach((f,l)=>{const w=r.children[l],h=w.querySelector("b:not(.out)");if(!h||h.textContent===f)return;if(!g){h.textContent=f;return}h.classList.add("out");const v=document.createElement("b");v.className="in",v.textContent=f,w.appendChild(v),setTimeout(()=>h.remove(),520)})}let c=!0;function d(){var p;const r=t-Date.now();if(r<=0){o.forEach(l=>u("[data-cd]",l).forEach(w=>i(w,"00",!1)));const f=(p=e.parentElement)==null?void 0:p.querySelector(".eyebrow");f&&(f.textContent="De Deuren Zijn Open");return}const m=Math.floor(r/1e3),g={days:n(Math.floor(m/86400)),hours:n(Math.floor(m%86400/3600)),minutes:n(Math.floor(m%3600/60)),seconds:n(m%60)};o.forEach(f=>{u("[data-cd]",f).forEach(l=>i(l,g[l.dataset.cd],!c))}),c=!1,setTimeout(d,1e3)}d()}function F(){const e=u(".acc-item");if(!e.length)return;function t(n){const i=s(".acc-panel",n);s(".acc-trigger",n).setAttribute("aria-expanded","false"),n.dataset.open="false",window.gsap&&!y?gsap.to(i,{height:0,duration:.4,ease:"power2.inOut"}):i.style.height="0px"}function o(n){const i=s(".acc-panel",n);s(".acc-trigger",n).setAttribute("aria-expanded","true"),n.dataset.open="true",window.gsap&&!y?gsap.to(i,{height:"auto",duration:.5,ease:"power3.out",onComplete:()=>window.ScrollTrigger&&ScrollTrigger.refresh()}):i.style.height="auto"}e.forEach(n=>{s(".acc-trigger",n).addEventListener("click",()=>{const i=n.dataset.open==="true";e.forEach(c=>c!==n&&t(c)),i?t(n):o(n)})})}function B(){u("[data-carousel-next]").forEach(t=>t.addEventListener("click",()=>e(t.dataset.carouselNext,1))),u("[data-carousel-prev]").forEach(t=>t.addEventListener("click",()=>e(t.dataset.carouselPrev,-1)));function e(t,o){const n=s("#carousel-"+t);if(!n||!n.firstElementChild)return;const i=parseFloat(getComputedStyle(n).gap)||16;n.scrollBy({left:o*(n.firstElementChild.getBoundingClientRect().width+i),behavior:y?"auto":"smooth"})}}function $(){var n;const e=s("#videoModal"),t=s("#modalVideo");if(!e||!t)return;u("[data-video]").forEach(i=>{i.addEventListener("click",()=>{t.src=i.dataset.video,e.showModal(),a&&a.stop(),t.play().catch(()=>{})})});function o(){t.pause(),t.removeAttribute("src"),t.load(),e.open&&e.close(),a&&a.start()}(n=s("[data-modal-close]",e))==null||n.addEventListener("click",o),e.addEventListener("close",()=>{t.pause(),a&&a.start()}),e.addEventListener("click",i=>{i.target===e&&o()})}function z(){const e=s("#stickyCta"),t=s("#hero");!e||!t||!("IntersectionObserver"in window)||new IntersectionObserver(([o])=>e.classList.toggle("is-visible",!o.isIntersecting),{threshold:0,rootMargin:"-80% 0px 0px 0px"}).observe(t)}function I(){window.matchMedia("(hover: none)").matches||u(".card-glow").forEach(e=>{e.addEventListener("mousemove",t=>{const o=e.getBoundingClientRect();e.style.setProperty("--mx",t.clientX-o.left+"px"),e.style.setProperty("--my",t.clientY-o.top+"px")})})}function V(){u("[data-cta]").forEach(e=>{e.addEventListener("click",()=>{const t={location:e.dataset.cta,value:125,currency:"EUR"};window.fbq&&fbq("track","InitiateCheckout",t),window.dataLayer&&dataLayer.push({event:"cta_click",...t})})})}function k(){C(),x(),T(),O(),N(),P(),D(),R(),F(),B(),$(),z(),I(),V(),window.mcReady=!0,document.dispatchEvent(new CustomEvent("mc:ready",{detail:{lenis:a}}))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",k):k()})();
;
